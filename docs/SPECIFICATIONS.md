...
2. L'offre publiée `offer` rejoint `offer_cible` par **pas proportionnel à l'écart restant** (≈ 50 % de l'écart, plancher ~25 W, plafonné à `soc_equaliser_probe_step_w`, symétrique, resets inclus) : **gros pas quand on est loin** (un grand écart de SoC se corrige en quelques mouvements), **pas doux près de l'équilibre** (anti-pompage). Le mouvement n'a lieu que **toutes les *N* ticks** ; *N* (`soc_equaliser_cadence_ticks`, plancher) est **dérivé du retard de réponse mesuré** de la batterie auto quand `soc_equaliser_adaptive_cadence` est actif (cadence ≫ temps mort du plant cloud).
3. L'offre est appliquée comme **plancher de décharge direct** sur la cible parc (`cible ≤ −offer` si `offer > 0`, càd au moins `offer` de décharge ; `cible ≥ −offer` si `offer < 0`, càd au moins `|offer|` de charge), **après** la zéro-injection. C'est volontaire : un simple biais du setpoint ZI **ne peut pas** forcer une décharge quand le parc pilotable **charge depuis son propre PV** (la cible net-réseau reste positive). Le plancher est **absolu** (clamp, pas d'ajout à la puissance mesurée → pas de runaway) ; l'offre étant proportionnelle, bornée et reculant sur fuite réseau, l'ensemble reste stable. La **zéro-injection** (P-seul) régule le réseau pour le reste. `offer > 0` = surplus offert (la batterie auto charge) ; `offer < 0` = déficit (elle décharge).
...
6. **Gate & plafond PV** : l'équaliseur n'agit que si la **production PV du parc pilotable** (ses propres MPPT) dépasse `soc_equaliser_min_pv_w` (défaut 200 W), et l'offre est **plafonnée à cette production**. Il ne redistribue donc que du **solaire** : jamais de transvasement batterie→batterie d'énergie stockée (perte aller-retour ~15-25 %), et la nuit il est inactif.
