...
class ActiveControlPublisher:
    ...
    def __init__(self, hass: HomeAssistant, devices: Sequence[Device]) -> None:
        self._hass = hass
        discharge_entities: dict[str, str] = {}
        for device in devices:
            ...
            if (
                ...
                discharge_entities[device.name] = battery.discharge_power_setpoint_entity
        self._discharge_entities = discharge_entities
        self._last_written: dict[str, float] = {}

    @property
    def enabled(self) -> bool:
        """True when at least one device declares a discharge setpoint entity."""
        return bool(self._discharge_entities)

    async def apply(self, per_battery_w: Mapping[str, float]) -> None:
        ...
        """
        for name, entity_id in self._discharge_entities.items():
    ...
    async def reset(self) -> None:
        ...
        """
        for entity_id in self._discharge_entities.values():
