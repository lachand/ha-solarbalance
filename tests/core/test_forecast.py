...
def test_build_slots_net_load_is_baseline_minus_pv() -> None:
    slots = build_forecast_slots(
        start=_START, n_hours=3, pv_w_by_hour=[1000.0, 500.0, 0.0],
        baseline_w=400.0, tariff=TariffConfig(),
    )
...
def test_pv_series_repeats_last_value() -> None:
    slots = build_forecast_slots(
        start=_START, n_hours=4, pv_w_by_hour=[800.0],
        baseline_w=300.0, tariff=TariffConfig(),
    )
...
def test_empty_pv_means_no_production() -> None:
    slots = build_forecast_slots(
        start=_START, n_hours=2, pv_w_by_hour=[], baseline_w=250.0, tariff=TariffConfig(),
    )
...
def test_prices_are_always_populated() -> None:
    slots = build_forecast_slots(
        start=_START, n_hours=2, pv_w_by_hour=[], baseline_w=0.0, tariff=TariffConfig(),
    )
...
def test_aggregate_sums_and_tightens_bounds() -> None:
    agg = aggregate_battery_constraints(
        [
            _bat(capacity_kwh=5.0, max_charge_w=2000.0, max_discharge_w=1800.0,
                 soc_min_pct=10.0, soc_max_pct=95.0),
            _bat(capacity_kwh=3.0, max_charge_w=1000.0, max_discharge_w=1200.0,
                 soc_min_pct=15.0, soc_max_pct=90.0),
        ]
    )
