...
- **Zero-injection regulation** — software PI controller targeting your grid meter (Shelly 3EM or any signed-power sensor at the PDL), with an anti-yoyo settle window when a big load drops. Optional **balance-point hysteresis** gives the loop two thresholds instead of one, sized from the actuator's own lag, so an error hovering at the deadband stops flipping it between regulating and idle every tick — widened on the import side only, never on export. Optional **actuator-lag loop tuning** derives the proportional gain from the same lag, so a slow box (STREAM ~30 s over BLE) starts calm instead of overshooting while commands are still in flight.
...
- **Anticipatory curtailment** — reactive curtailment can only trim PV *after* an export is measured, so a fast solar rise into a full fleet spills a transient. SolarBalance projects a **sink budget** (all batteries — cloud ones included — plus the commandable loads) against the PV+consumption forecast and pre-brakes the inverter *before* the surplus lands. It never throws away solar while a sink still has room, and only ever lowers the PV limit (opt-in).
...
- **Installation health score** — one 0-100 number aggregating link health, the plausibility guard, config issues, meter source and degraded mode, with the reasons for any shortfall — so "should I be worried?" is one glance.
- **Anomaly timeline** — a short, de-duplicated log of the notable incidents (meter lost/recovered, impossible reading rejected, link down, unusual appliance cycle) in one place, instead of scrolling the logbook.
...
- **Appliance cycle learning** — declare washing-machine / dishwasher power sensors and SolarBalance learns each program's power curve, energy and duration, then advises when running one would be covered by the sun, **and what it costs** (euros now vs euros saved by waiting for the sun, at your tariff). On a smart-plug setup (no program signal) cycles all start as `unknown`; **label them from the panel** — rename a whole bucket, or name just the last wash you ran so a 40° and a 20° separate one cycle at a time.
- **Battery health from real throughput** — measured **round-trip efficiency** (energy out / energy in, corrected for the charge still stored) and **equivalent full cycles**, giving a State-of-Health estimate even for packs that expose no vendor cycle count.
- **HA services** — `pause`, `resume`, `force_charge`, `force_discharge`, `force_charge_load`, `cancel_force_charge_load`, `set_mode`, `activate_storm_mode`, `rename_appliance_program`, `label_last_cycle` callable from automations or the dashboard.
